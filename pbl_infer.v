module pbl_infer (
    input  wire        clk,
    input  wire        reset,
    input  wire        start,           

    output reg  [16:0] w_addr,
    input  wire [15:0] w_q,
    output reg  [6:0]  b_addr,
    input  wire [15:0] b_q,
    output reg  [10:0] beta_rd_addr,
    input  wire [15:0] beta_rd_q,
    output reg  [9:0]  img_rd_addr,
    input  wire [15:0] img_rd_q,
    output reg  [3:0]  pred,
    output reg         done,            
    output reg         error            
);

    localparam integer D      = 784;
    localparam integer H      = 128;
    localparam integer C      = 10;
    localparam integer DATA_W = 16;
    localparam integer ACC_W  = 32;
    localparam integer Q_FRAC = 12;

    localparam [4:0] PH_IDLE         = 5'd0;
    localparam [4:0] PH_CLEAR        = 5'd1;  
    localparam [4:0] PH_H_ADDR       = 5'd2;
    localparam [4:0] PH_H_WAIT0      = 5'd3;
    localparam [4:0] PH_H_WAIT1      = 5'd4;
    localparam [4:0] PH_H_MAC        = 5'd5;
    localparam [4:0] PH_H_BIAS       = 5'd6;
    localparam [4:0] PH_H_BIAS_WAIT0 = 5'd7;
    localparam [4:0] PH_H_BIAS_WAIT1 = 5'd8;
    localparam [4:0] PH_H_TANH       = 5'd9;
    localparam [4:0] PH_H_TANH_LATCH = 5'd10;
    localparam [4:0] PH_O_ADDR       = 5'd11;
    localparam [4:0] PH_O_WAIT0      = 5'd12;
    localparam [4:0] PH_O_WAIT1      = 5'd13;
    localparam [4:0] PH_O_MAC        = 5'd14;
    localparam [4:0] PH_ARGMAX       = 5'd15;

    reg [4:0] phase;

    reg [$clog2(D)-1:0] in_idx;
    reg [$clog2(H)-1:0] hid_idx;
    reg [$clog2(C)-1:0] cls_idx;
    reg [$clog2(H)-1:0] clear_idx;  

    reg signed [DATA_W-1:0] h_mem [0:H-1];
    reg signed [ACC_W-1:0]  y_mem [0:C-1];

    reg signed [ACC_W-1:0] acc;
    reg signed [ACC_W-1:0] z_hidden;

    // =========================================================================
    // Endereços derivados de hid_idx
    // hid_x784 = hid_idx * 784 = hid_idx*(512+256+16)
    // hid_x10  = hid_idx * 10  = hid_idx*(8+2)
    // =========================================================================
    wire [16:0] hid_x784 = ({10'b0, hid_idx} << 9)
                          + ({10'b0, hid_idx} << 8)
                          + ({10'b0, hid_idx} << 4);

    wire [10:0] hid_x10  = ({4'b0, hid_idx} << 3)
                          + ({4'b0, hid_idx} << 1);

    // =========================================================================
    // MACs combinacionais
    // =========================================================================
    wire signed [ACC_W-1:0] mult_hidden_full,  mult_hidden_scaled;
    wire signed [ACC_W-1:0] mult_output_full,  mult_output_scaled;

    mac_q412 #(.DATA_W(DATA_W), .ACC_W(ACC_W), .Q_FRAC(Q_FRAC)) u_mac_hidden (
        .a              (img_rd_q),
        .b              (w_q),
        .product_full   (mult_hidden_full),
        .product_scaled (mult_hidden_scaled)
    );

    mac_q412 #(.DATA_W(DATA_W), .ACC_W(ACC_W), .Q_FRAC(Q_FRAC)) u_mac_output (
        .a              (h_mem[hid_idx]),
        .b              (beta_rd_q),
        .product_full   (mult_output_full),
        .product_scaled (mult_output_scaled)
    );

    // =========================================================================
    // Saturação 32 → 16 bits antes da tanh
    // =========================================================================
    wire signed [DATA_W-1:0] z_sat;
    function signed [DATA_W-1:0] sat32_to_q16;
        input signed [ACC_W-1:0] x;
        begin
            if      (x > 32'sd32767)  sat32_to_q16 =  16'sd32767;
            else if (x < -32'sd32768) sat32_to_q16 = -16'sd32768;
            else                      sat32_to_q16  =  x[DATA_W-1:0];
        end
    endfunction
    assign z_sat = sat32_to_q16(z_hidden);

    // =========================================================================
    // Tanh (approximação piecewise linear em Q4.12)
    // =========================================================================
    wire signed [DATA_W-1:0] tanh_out;
    tanh_lut #(.DATA_W(DATA_W), .Q_FRAC(Q_FRAC)) u_tanh (
        .x_in  (z_sat),
        .y_out (tanh_out)
    );

    // =========================================================================
    // Argmax sobre as 10 saídas
    // =========================================================================
    wire [3:0]              pred_argmax;
    wire signed [ACC_W-1:0] max_val_unused;
    argmax10 #(.ACC_W(ACC_W)) u_argmax (
        .y0(y_mem[0]), .y1(y_mem[1]), .y2(y_mem[2]), .y3(y_mem[3]), .y4(y_mem[4]),
        .y5(y_mem[5]), .y6(y_mem[6]), .y7(y_mem[7]), .y8(y_mem[8]), .y9(y_mem[9]),
        .pred    (pred_argmax),
        .max_val (max_val_unused)
    );

    // =========================================================================
    // FSM principal
    // =========================================================================
    integer i;
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            phase        <= PH_IDLE;
            done         <= 0;
            error        <= 0;
            pred         <= 0;
            in_idx       <= 0;
            hid_idx      <= 0;
            cls_idx      <= 0;
            clear_idx    <= 0;
            acc          <= 0;
            z_hidden     <= 0;
            w_addr       <= 0;
            b_addr       <= 0;
            beta_rd_addr <= 0;
            img_rd_addr  <= 0;
            // Inicialização no reset é segura — só executada uma vez
            for (i = 0; i < H; i = i + 1) h_mem[i] <= 0;
            for (i = 0; i < C; i = i + 1) y_mem[i] <= 0;
        end else begin
            done  <= 0;
            error <= 0;

            case (phase)

                // ---------------------------------------------------------
                // IDLE: aguarda pulso de start
                // ---------------------------------------------------------
                PH_IDLE: begin
                    if (start) begin
                        acc       <= 0;
                        z_hidden  <= 0;
                        pred      <= 0;
                        clear_idx <= 0;
                        phase     <= PH_CLEAR;
                    end
                end

                // ---------------------------------------------------------
                // CLEAR: limpa h_mem e y_mem 1 posição por ciclo
                // Garante buffers limpos sem depender do for loop
                // H=128 ciclos — desprezível frente ao cálculo
                // ---------------------------------------------------------
                PH_CLEAR: begin
                    h_mem[clear_idx] <= 0;
                    if (clear_idx < C)
                        y_mem[clear_idx[3:0]] <= 0;
                    if (clear_idx == H-1) begin
                        in_idx  <= 0;
                        hid_idx <= 0;
                        cls_idx <= 0;
                        phase   <= PH_H_ADDR;
                    end else begin
                        clear_idx <= clear_idx + 1'b1;
                    end
                end

                // ---------------------------------------------------------
                // Camada oculta: MAC sobre D=784 entradas para cada hid_idx
                // ---------------------------------------------------------
                PH_H_ADDR: begin
                    img_rd_addr <= in_idx[9:0];
                    w_addr      <= hid_x784 + {7'b0, in_idx};
                    phase       <= PH_H_WAIT0;
                end
                PH_H_WAIT0: phase <= PH_H_WAIT1;
                PH_H_WAIT1: phase <= PH_H_MAC;
                PH_H_MAC: begin
                    acc <= acc + mult_hidden_scaled;
                    if (in_idx == D-1) begin
                        in_idx <= 0;
                        phase  <= PH_H_BIAS;
                    end else begin
                        in_idx <= in_idx + 1'b1;
                        phase  <= PH_H_ADDR;
                    end
                end

                // ---------------------------------------------------------
                // Bias da camada oculta + tanh
                // ---------------------------------------------------------
                PH_H_BIAS: begin
                    b_addr <= hid_idx[6:0];
                    phase  <= PH_H_BIAS_WAIT0;
                end
                PH_H_BIAS_WAIT0: phase <= PH_H_BIAS_WAIT1;
                PH_H_BIAS_WAIT1: phase <= PH_H_TANH;
                PH_H_TANH: begin
                    // z_hidden registrado → z_sat e tanh_out combinacionais
                    // prontos no ciclo seguinte (PH_H_TANH_LATCH)
                    z_hidden <= acc + {{(ACC_W-DATA_W){b_q[DATA_W-1]}}, b_q};
                    phase    <= PH_H_TANH_LATCH;
                end
                PH_H_TANH_LATCH: begin
                    h_mem[hid_idx] <= tanh_out;
                    acc            <= 0;
                    z_hidden       <= 0;
                    if (hid_idx == H-1) begin
                        hid_idx <= 0;
                        cls_idx <= 0;
                        phase   <= PH_O_ADDR;
                    end else begin
                        hid_idx <= hid_idx + 1'b1;
                        phase   <= PH_H_ADDR;
                    end
                end

                // ---------------------------------------------------------
                // Camada de saída: MAC sobre H=128 neurônios para cada cls
                // ---------------------------------------------------------
                PH_O_ADDR: begin
                    beta_rd_addr <= hid_x10 + {7'b0, cls_idx};
                    phase        <= PH_O_WAIT0;
                end
                PH_O_WAIT0: phase <= PH_O_WAIT1;
                PH_O_WAIT1: phase <= PH_O_MAC;
                PH_O_MAC: begin
                    if (hid_idx == H-1) begin
                        y_mem[cls_idx] <= acc + mult_output_scaled;
                        acc            <= 0;
                        hid_idx        <= 0;
                        if (cls_idx == C-1) begin
                            cls_idx <= 0;
                            phase   <= PH_ARGMAX;
                        end else begin
                            cls_idx <= cls_idx + 1'b1;
                            phase   <= PH_O_ADDR;
                        end
                    end else begin
                        acc     <= acc + mult_output_scaled;
                        hid_idx <= hid_idx + 1'b1;
                        phase   <= PH_O_ADDR;
                    end
                end

                // ---------------------------------------------------------
                // Argmax: lê resultado combinacional e sinaliza done
                // ---------------------------------------------------------
                PH_ARGMAX: begin
                    pred  <= pred_argmax;
                    done  <= 1;
                    phase <= PH_IDLE;
                end

                default: begin
                    error <= 1;
                    phase <= PH_IDLE;
                end

            endcase
        end
    end
endmodule